        ...
        for i, part in enumerate(filename, 1):
            ...
            if thumb in ["/d", "no"] or not os.path.exists(thumb):
                ...
                if os.path.exists(temp_thumb):
                    ...
                    text_cmd = f'ffmpeg -i "{temp_thumb}" -vf "drawbox=y=0:color=black@0.3:width=iw:height=200:t=fill,drawtext=fontfile=font.otf:text=\'{spaced_text}\':fontcolor=white:fontsize=90:x=(w-text_w)/2:y=(h-text_h)/2" -c:v mjpeg -q:v 2 -y "{temp_thumb}"'
